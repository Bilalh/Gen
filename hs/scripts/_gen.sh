...
_gen()
{
    ...
    if [ "${last}x" = "genx" ]; then
        COMPREPLY=( $(compgen -W "essence reduce generalise solve link json weights script-toolchain script-recheck script-updateChoices script-removeDups script-createDbHashes" -- $cur) )
        return 0
    fi
