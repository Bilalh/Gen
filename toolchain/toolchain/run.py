...
class Status(Enum):
    ...
    typeError                = 22,
    enumerateDomain          = 23,
    parseErrorUndefined      = 24,
    conjureUserError         = 25,
    conjureShouldNeverHappen = 26,
    notAConstant             = 27,
    emptyHandling            = 28,
    refineUnreferencedVar    = 29,
    domainUnion              = 30,
    outOfBoundsIndexing      = 31,
    categoryChecking         = 32,
    logFollowing             = 33,
...
def classify_error(*, kind, output, returncode):
    ...
    if kind == K.savilerow:
        ...
        and ('Sub-process exited with error code:137' in output or 'Sub-process exited with error code:139' in output):
